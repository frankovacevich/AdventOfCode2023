...
func labelHandWithJokers(hand *Hand) {

	for i := range Letters {
		newCards := strings.Replace(hand.cards, "J", string(Letters[i]), -1)
		newHand := Hand{cards: newCards}
		labelHand(&newHand)
		if newHand.label > hand.label {
			hand.label = newHand.label
		}
	}
}
